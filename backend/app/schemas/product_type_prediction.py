"""
Pydantic schemas for product type-based lapse prediction endpoint.
"""
...
class ProductTypePredictionRequest(BaseModel):
    target_month: MonthStr = Field(..., description="Prediction month in YYYY-MM format")


class ProductTypePaidPct(BaseModel):
    product_type: str
    avg_paid_pct: float
...
class ReferenceMonthDetail(BaseModel):
    month: MonthStr
    product_type_paid_pct: List[ProductTypePaidPct]
...
class ProductTypePrediction(BaseModel):
    product_type: str
    historical_avg_paid_pct: float
...
class ProductTypePredictionResponse(BaseModel):
    target_month: MonthStr
    reference_months: List[MonthStr]
    summary: PredictionSummary
    by_product_type: List[ProductTypePrediction]
