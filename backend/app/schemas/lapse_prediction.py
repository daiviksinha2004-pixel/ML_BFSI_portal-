...
class ProductGroupPaidPct(BaseModel):
    product_group: str
    avg_paid_pct: float
...
class ReferenceMonthDetail(BaseModel):
    month: MonthStr
    product_group_paid_pct: List[ProductGroupPaidPct]
...
class ProductGroupPrediction(BaseModel):
    product_group: str
    historical_avg_paid_pct: float
...
class LapsePredictionResponse(BaseModel):
    ...
    by_product_group: List[ProductGroupPrediction]
