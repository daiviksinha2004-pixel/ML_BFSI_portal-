"""
Service layer for channel-based lapse prediction endpoint.
Implements business logic for predicting paid policies based on historical data, grouped by channel.
"""
...
from app.schemas.channel_prediction import (
...
class ChannelPredictionService:
    """Service for channel-based lapse prediction and forecasting."""
    ...
    def fetch_month_data(self, month: str, tenant_id: str) -> pd.DataFrame:
        ...
        Returns DataFrame with columns: channel, policy_issue_date, paid_to_date, pmt_flag, act_premium
        """
    ...
    def compute_paid_counts_by_group(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Group by channel → lapse_aging_band
        Calculate total and paid counts for each cell
        """
        if df.empty:
            return pd.DataFrame(columns=['channel', 'lapse_aging_band', 'total_count', 'paid_count'])
        
        # Group by (channel, lapse_aging_band)
        grouped = df.groupby(['channel', 'lapse_aging_band']).agg({
            'pmt_flag': ['count', 'sum']
        }).reset_index()
        
        grouped.columns = ['channel', 'lapse_aging_band', 'total_count', 'paid_count']
        
        return grouped
    
    def get_avg_ticket_size_by_channel(
        self,
        months: List[str],
        tenant_id: str
    ) -> Dict[str, float]:
        """
        Calculate average ticket size (premium per policy) per channel across given months.
        ...
        Returns:
            Dict mapping channel to average ticket size (float)
        """
        if not months:
            return {}
        ...
        # Build query grouped by channel
        query = self.db.query(
            LifeCampaignRecord.channel,
        ...
        for row in results:
            ...
            if total_policies > 0:
                ticket_sizes[ch] = float((row.total_premium or 0) / total_policies)
            else:
                ticket_sizes[ch] = 0.0
                
        return ticket_sizes

    def get_robust_ticket_size_by_channel(
        self,
        months: List[str],
        tenant_id: str
    ) -> Dict[str, float]:
        ...
        for month in months:
            ...
            for ch, grp in paid_df.groupby('channel'):
                values = grp['effective_ticket'].tolist()
                by_ch.setdefault(ch or 'Unknown', []).extend(values)

        robust_ticket_sizes: Dict[str, float] = {}
        for ch, values in by_ch.items():
            ...
            robust_ticket_sizes[ch] = float(np.percentile(arr, 60))
    ...
    def get_historical_avg_paid_pct(
        ...
        Returns:
        1. historical_avg_by_ch: Recency-weighted avg per channel
        2. historical_avg_by_band: Recency-weighted avg per (channel, lapse_aging_band)
        ...
        band_totals = {}  # (ch, band) -> {'total': 0, 'paid': 0}
        ch_totals = {}    # ch -> {'total': 0, 'paid': 0}
        ...
        for month_idx, month in enumerate(reference_months):
            ...
            try:
                ...
                # Compute channel monthly averages for reference details
                monthly_ch_stats = {}
                for _, row in counts_df.iterrows():
                    ...
                    if ch not in monthly_ch_stats:
                        monthly_ch_stats[ch] = {'total': 0, 'paid': 0}
                    monthly_ch_stats[ch]['total'] += total
                    monthly_ch_stats[ch]['paid'] += paid
                    
                    # Accumulate with recency weight
                    if ch not in ch_totals:
                        ch_totals[ch] = {'total': 0.0, 'paid': 0.0}
                    ch_totals[ch]['total'] += total * w
                    ch_totals[ch]['paid']  += paid  * w
                ...
                channel_paid_pct = []
                for ch, stats in monthly_ch_stats.items():
                    pct = (stats['paid'] / stats['total'] * 100) if stats['total'] > 0 else 0
                    channel_paid_pct.append(ChannelPaidPct(channel=ch, avg_paid_pct=round(pct, 2)))
                
                reference_details.append(ReferenceMonthDetail(month=month, channel_paid_pct=channel_paid_pct))
        ...
        historical_avg_by_ch = {}
        for ch, stats in ch_totals.items():
            raw_rate = (stats['paid'] / stats['total'] * 100) if stats['total'] > 0 else 0
            historical_avg_by_ch[ch] = raw_rate
        ...
        return historical_avg_by_ch, historical_avg_by_band, reference_details
    ...
    def get_target_month_policy_counts(
        ...
        Group by (channel, lapse_aging_band) → get policy_count
        ...
        # Group by (channel, lapse_aging_band)
        grouped = df.groupby(['channel', 'lapse_aging_band']).size().reset_index(name='policy_count')
    ...
    def generate_prediction(
        ...
        historical_avg_by_ch, historical_avg_by_band, reference_details = self.get_historical_avg_paid_pct(
            reference_months, tenant_id
        )
        
        if not historical_avg_by_ch:
            raise ValueError("No historical data available for reference months")
        
        # STEP 5.5: Get average ticket size per channel
        ticket_sizes = self.get_robust_ticket_size_by_channel(
        ...
        for channel in target_counts['channel'].unique():
            ch_data = target_counts[target_counts['channel'] == channel]
            
            # This is only used for UI display, actual prediction happens at band level
            overall_hist_paid_pct = historical_avg_by_ch.get(channel, 0)
            ticket_size = ticket_sizes.get(channel, 0.0)
            
            lapse_band_breakdown = []
            total_predicted_paid_ch = 0
            total_policy_count_ch = 0
            total_predicted_amount_ch = 0
            
            for _, row in ch_data.iterrows():
                lapse_band = row['lapse_aging_band']
                policy_count = row['policy_count']
                
                # Fetch band-level historical rate (fallback to channel rate if unseen)
                band_hist_paid_pct = historical_avg_by_band.get((channel, lapse_band), overall_hist_paid_pct)
                ...
                total_predicted_paid_ch += predicted_paid
                total_policy_count_ch += policy_count
                total_predicted_amount_ch += predicted_amount
            
            predicted_paid_pct_ch = (total_predicted_paid_ch / total_policy_count_ch * 100) if total_policy_count_ch > 0 else 0
            
            by_channel.append(ChannelPrediction(
                channel=channel,
                historical_avg_paid_pct=round(overall_hist_paid_pct, 2),
                total_policy_count=total_policy_count_ch,
                predicted_paid_count=round(total_predicted_paid_ch, 2),
                predicted_paid_pct=round(predicted_paid_pct_ch, 2),
                predicted_collected_amount=round(total_predicted_amount_ch, 2),
                lapse_band_breakdown=lapse_band_breakdown
            ))
            
            overall_predicted_paid += total_predicted_paid_ch
            overall_total_policies += total_policy_count_ch
            overall_predicted_amount += total_predicted_amount_ch
        ...
        return ChannelPredictionResponse(
